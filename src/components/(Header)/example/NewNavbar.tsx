...
const NewNavbar = ({ locale }: { locale: string }) => {
    ...
    return (
        <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-md">
            <nav className="max-w-screen-2xl 2xl:max-w-screen-xl maxWeb:max-w-screen-2xl mx-auto flex items-center justify-between px-6 py-4 lg:px-16 3xl:py-6 maxWeb:py-8">
                ...
                <ul className="hidden md:flex space-x-8">
                    {menuItems.map((item) => (
                        ...
                        >
                            <button className="text-gray-700 hover:text-red-600 px-4 3xl:text-xl maxWeb:text-2xl font-semibold">
                                {item.label}
                            </button>
                        </li>
                    ))}
