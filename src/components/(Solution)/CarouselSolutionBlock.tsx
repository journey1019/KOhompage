...
const CarouselSolutionBlock: React.FC<CarouselProps> = ({ items }) => {
    ...
    return (
        <div className="relative w-full max-w-7xl maxWeb:max-w-screen-2xl mx-auto overflow-hidden pb-10">
                    ...
                    >
                        ...
                        <div
                            ...
                            {[item.imageUrl1, item.imageUrl2, item.imageUrl3].map((url, imgIndex) => (
                                <div
                                    key={imgIndex}
                                    className="relative w-full aspect-[7/4] smallMobile:aspect-[7/4] bigMobile:aspect-[9/3] lg:aspect-[4/3] xl:aspect-[16/12] maxWeb:aspect-[4/5] rounded-xl overflow-hidden transition-transform transform hover:scale-105 hover:border-2 hover:border-blue-500"
                                >
                                    <Image
                                        src={url}
            ...
            <button
                onClick={handlePrev}
                className="absolute text-base maxWeb:text-2xl top-1/2 left-4 transform -translate-y-1/2 bg-gray-800 bg-opacity-70 text-white py-2 px-4 maxWeb:py-3 maxWeb:px-5 rounded-full shadow-lg hover:bg-gray-600"
            >
                ❮
            </button>
            <button
                onClick={handleNext}
                className="absolute text-base maxWeb:text-2xl top-1/2 right-4 transform -translate-y-1/2 bg-gray-800 bg-opacity-70 text-white py-2 px-4 maxWeb:py-3 maxWeb:px-5 rounded-full shadow-lg hover:bg-gray-600"
            >
                ❯
            </button>
            ...
            <div
                ...
                {items.map((_, index) => (
                    <button
                        ...
                        className={`h-2 maxWeb:h-3 w-10 sm:w-1/5 maxWeb:w-1/4 rounded-full transition-all duration-300 ${
