...
export default function Characteristics({ items, gridCols = 4 }: CharacteristicsProps) {
    ...
    return (
        <section className="mx-auto max-w-7xl maxWeb:max-w-screen-2xl px-6 lg:px-8 pb-12">
            <motion.div
                className={`grid grid-cols-1 gap-8 maxWeb:gap-4 mt-16 md:grid-cols-2 lg:grid-cols-${gridCols} max-w-7xl mx-auto px-6`}
                ...
                {items.map((item, index) => (
                    ...
                    >
                        <Image
                            ...
                            unoptimized
                        />
                        <h2 className="text-lg maxWeb:text-2xl text-gray-600 text-center font-semibold">
                            {item.title}
                        </h2>
                        <p className="mt-2 text-sm maxWeb:text-lg text-gray-500 dark:text-gray-400 text-center line-clamp-3">
                            {item.subtitle}
                        </p>
                    </motion.div>
                ))}
