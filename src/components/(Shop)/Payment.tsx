...
export default function Payment() {
    ...
    const [userInfo, setUserInfo] = useState({
        ...
        tel: '010-1234-5678',
        addr: '서울특별시 강남대로 525 세영제이타원 15층',
        postcode: '12345',
    })
    ...
    const handlePayment = () => {
        ...
        IMP.request_pay({
            ...
            // customer_uid: 'user_test_001', // 정기결제에 필요
    ...
    const executeBilling = async () => {
        const res = await fetch('/api/portone/charge', {
            method: 'POST',
            body: JSON.stringify({
                customer_uid: 'user_01012345678',
                amount: 9900,
                name: '데이터 요금제 자동결제',
                buyer_email: 'jhlee@gmail.com',
                buyer_name: '이지현',
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div className="mt-6 flex gap-4 justify-end">
                            <button className="px-4 py-2 rounded border cursor-pointer hover:bg-gray-100"
                                    onClick={() => setOpen(false)}>취소
                            </button>
                            <button className="px-4 py-2 rounded bg-blue-600 text-white cursor-pointer hover:bg-blue-700"
                                    onClick={handlePayment}>결제하기
                            </button>
                            <button className="px-4 py-2 rounded bg-blue-600 text-white cursor-pointer"
                                    onClick={executeBilling}>정기결제
                            </button>
                        </div>
                    </div>
                        ...
                        {cart.map(product => (
                            <div key={product.id} className="flex items-center justify-between mb-4">
                                ...
                                <button onClick={() => removeFromCart(product.id)}
                                        className="text-red-500 text-sm hover:underline cursor-pointer">삭제
                                </button>
                            </div>
                        ))}
